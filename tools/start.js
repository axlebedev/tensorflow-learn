...
async function start() {
  ...
  const clientConfig = webpackConfig.find((config) => {
    return config.name === 'client'
  })
  clientConfig.entry.client = ['./tools/lib/webpackHotDevClient']
    .concat(clientConfig.entry.client)
    .sort((a, b) => {
      return b.includes('polyfill') - a.includes('polyfill')
    })
  ...
  clientConfig.module.rules = clientConfig.module.rules.filter(
    (x) => {
      return x.loader !== 'null-loader'
    },
  )
  clientConfig.plugins.push(new webpack.HotModuleReplacementPlugin())

  // Configure server-side hot module replacement
  const serverConfig = webpackConfig.find((config) => {
  ...
  serverCompiler.hooks.compile.tap('server', () => {
    ...
    appPromise = new Promise((resolve) => {
      ...
      return (appPromiseResolve = resolve)
    })
  })
      ...
      .then((updatedModules) => {
        ...
        if (updatedModules.length === 0) {
          ...
          updatedModules.forEach((moduleId) => {
            return console.info(`${hmrPrefix} - ${moduleId}`)
          },
          )
          checkForUpdate(true)
        }
